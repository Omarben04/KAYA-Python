...
📐 L'Équation de Kaya
CO2=CO2TOE×TOEGDP×GDPPOP×POPCO_2 = \frac{CO_2}{TOE} \times \frac{TOE}{GDP} \times \frac{GDP}{POP} \times POPCO2​=TOECO2​​×GDPTOE​×POPGDP​×POP
TermeSignificationCO₂Émissions totales de dioxyde de carboneTOEConsommation totale d'énergie (tonnes équivalent pétrole)GDPProduit Intérieur Brut mondialPOPPopulation mondiale
...
BibliothèqueRôlepandasManipulation, filtrage et traitement des DataFramesmatplotlibTracé et mise en forme du graphique final

🚀 Utilisation

Placez les 4 fichiers .csv dans le même répertoire que kaya.py.
Ouvrez kaya.py dans votre IDE (ex : Spyder, VS Code).
Vérifiez que le répertoire de travail de l'IDE pointe bien vers ce dossier.
Exécutez le script.
...
pythonCO2df = pd.read_csv('...csv', header=2)  # header=2 ignore les métadonnées Banque Mondiale
2. Extraction des données mondiales
pythonCO2 = CO2df[CO2df['Country Code'] == 'WLD'][annees].squeeze()
3. Construction du DataFrame principal
pythondata = pd.DataFrame({'CO2': CO2, 'GDP': GDP, 'POP': POP})
data['TOE'] = TOE_over_POP * POP
4. Calcul en base 100 (référence : 1971)
pythondata['CO2_b100'] = 100 * data['CO2'] / data['CO2']['1971']
5. Visualisation
pythonextraction_kaya.plot(logy=True, figsize=(10, 6))
